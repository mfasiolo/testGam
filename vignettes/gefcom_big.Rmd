...
```
EDF now lower than `k'` for all variables, apart from `Tendance` (but this is ok, see above).
...
The following code produces some RGL plots, here we show only the last on:
```{r bam14, message = F, webgl=TRUE}
